...
// メタデータ抽出用のプロンプト（Readable形式PDF用）
export const READABLE_META_EXTRACTION_PROMPT = `
この論文PDFは、奇数ページに日本語訳、偶数ページに英語原文が配置されています。
...
// 要約用のデフォルトプロンプト（Readable形式PDF用）
export const READABLE_SUMMARY_PROMPT = `
与えられた論文のPDF資料の内容のみに基づき、以下の見出しに対応する内容をそれぞれ簡潔に要約してください。
このPDF資料は、奇数ページに日本語訳、偶数ページに英語原文が配置されています。
要約の際には、偶数ページの英語原文と奇数ページの日本語訳の両方を参考にし、元の内容を正確に反映するように努めてください。
