...
### ステップ2: ローカルファイル対応設定（オプション）

ローカルPDFファイル（file://スキーム）を処理する場合は、以下の設定が必要です：

1. **Chrome拡張機能ページを開く**
   - アドレスバーに `chrome://extensions/` と入力してEnter

2. **Paper2Notion拡張機能を見つける**
   - 拡張機能一覧からPaper2Notionを探す
...
5. **設定完了**
   - これでfile://スキームのローカルファイルが処理可能になります
...
- **重要**: 拡張機能の設定で「ファイルのURLへのアクセスを許可する」を有効にする必要があります
...
#### 「ローカルファイルにアクセスできません」（file://スキーム）
- **原因**: 拡張機能にfile://スキームへのアクセス権限が設定されていない
- **解決方法**:
